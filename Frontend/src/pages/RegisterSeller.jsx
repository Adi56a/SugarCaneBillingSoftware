...
import { FaUser, FaPhone, FaUserPlus, FaLanguage, FaCheck, FaTimes, FaStore } from 'react-icons/fa';

// Language translations for EN and Marathi
const translations = {
  en: {
    title: "Register New Seller",
    subtitle: "Add sellers to your sugar mill management system",
    seller_name: "Seller Name",
    seller_number: "Seller Mobile Number",
    register_button: "Register Seller",
    registering: "Registering...",
    success_message: "Seller registered successfully",
    failure_message: "Error registering seller",
    already_exists_message: "Seller with this number already exists",
    required_fields: "Seller name and number both are required",
    token_error: "Authorization failed. Please login again.",
    server_error: "Server error, please try again later.",
    name_placeholder: "Enter seller's full name",
    number_placeholder: "Enter 10-digit mobile number",
    switchLanguage: "Switch to Marathi",
    secureRegistration: "Secure Registration",
    registrationInfo: "All seller data is securely stored and encrypted"
  },
  mr: {
    title: "नवीन विक्रेता नोंदणी",
    subtitle: "तुमच्या साखर कारखाना व्यवस्थापन सिस्टममध्ये विक्रेता जोडा",
    seller_name: "विक्रेत्याचे नाव",
    seller_number: "विक्रेत्याचा मोबाइल नंबर",
    register_button: "विक्रेता नोंदवा",
    registering: "नोंदणी करत आहे...",
    success_message: "विक्रेता यशस्वीरित्या नोंदवला गेला",
    failure_message: "विक्रेता नोंदवताना त्रुटी",
    already_exists_message: "या नंबरसह विक्रेता आधीच अस्तित्वात आहे",
    required_fields: "विक्रेत्याचे नाव आणि नंबर आवश्यक आहेत",
    token_error: "अधिकार प्रमाणीकरण अयशस्वी. कृपया पुन्हा लॉगिन करा.",
    server_error: "सर्व्हर त्रुटी, कृपया नंतर पुन्हा प्रयत्न करा.",
    name_placeholder: "विक्रेत्याचे पूर्ण नाव टाका",
    number_placeholder: "10 अंकी मोबाइल नंबर टाका",
    switchLanguage: "इंग्रजीमध्ये बदला",
    secureRegistration: "सुरक्षित नोंदणी",
    registrationInfo: "सर्व विक्रेता डेटा सुरक्षितपणे संग्रहीत आणि एन्क्रिप्ट केला जातो"
  }
};
...
const RegisterSeller = () => {
  ...
  // Validation effects
  useEffect(() => {
    setNameValid(sellerName.trim().length >= 2);
  }, [sellerName]);

  useEffect(() => {
    const phoneRegex = /^[6-9]\d{9}$/;
    setNumberValid(phoneRegex.test(sellerNumber));
  }, [sellerNumber]);
  ...
  const validateForm = () => {
    ...
    if (sellerName.trim().length < 2) {
      showMessageWithType(
        language === 'en' ? 'Seller name must be at least 2 characters' : 'विक्रेत्याचे नाव किमान 2 अक्षरांचे असावे',
        "error"
      );
      return false;
    }

    if (!sellerNumber.trim()) {
      showMessageWithType(t.required_fields, "error");
      return false;
    }

    const phoneRegex = /^[6-9]\d{9}$/;
    if (!phoneRegex.test(sellerNumber)) {
      showMessageWithType(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const baseUrl = process.env.NODE_ENV === 'development' 
        ...
        : 'https://ltkpt3jvkqurdf3vh7gq37cgby0adqsm.lambda-url.ap-south-1.on.aws/api/seller/register'; // Replace with your actual production URL
      ...
      const response = await fetch(baseUrl, {
        ...
        body: JSON.stringify({ seller_name: sellerName, seller_number: sellerNumber })
      });
        ...
        <div className="max-w-md w-full space-y-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full pl-10 pr-10 py-3 border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-200 transition-all duration-300 ${
                      sellerName ? (nameValid ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500') : 'border-gray-300 focus:border-purple-500'
                    }`}
                    required
                    disabled={isLoading}
                  />
              ...
              {/* Seller Number Field */}
              <div>
                <label htmlFor="seller_number" className="block text-sm font-semibold text-gray-700 mb-2">
                  {t.seller_number}
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  ...
                  <input
                    ...
                    id="seller_number"
                    value={sellerNumber}
                    onChange={(e) => setSellerNumber(e.target.value.replace(/\D/g, '').slice(0, 10))}
                    placeholder={t.number_placeholder}
                    className={`w-full pl-10 pr-10 py-3 border-2 rounded-xl focus:outline-none focus:ring-4 focus:ring-purple-200 transition-all duration-300 ${
                      sellerNumber ? (numberValid ? 'border-green-500 focus:border-green-500' : 'border-red-500 focus:border-red-500') : 'border-gray-300 focus:border-purple-500'
                    }`}
                    required
                    disabled={isLoading}
                    maxLength={10}
          ...
          <div className="text-center">
            <button
              ...
              className="flex items-center space-x-2 mx-auto px-6 py-3 bg-white text-gray-700 rounded-full shadow-md hover:shadow-lg border border-gray-200 hover:border-purple-300 transition-all duration-300"
            >
